...
export default async function BillingPage() {
  ...
  const [cycles, cards] = await Promise.all([
    prisma.billingCycle.findMany({
      where: { card: { userId } },
      include: {
        card: { select: { nickname: true, bankName: true, billingDate: true, dueDate: true } },
        transactions: {
    ...
    prisma.creditCard.findMany({
      ...
      select: { id: true, nickname: true, bankName: true, billingDate: true, dueDate: true, creditLimit: true },
    }),
  ])
  ...
  const totalOutstanding = unpaid.reduce((sum, c) => sum + Number(c.totalSpend) - Number(c.paidAmount), 0)
  ...
  return (
    <div className="space-y-6 pb-8">
      <div>
        <h1 className="text-2xl font-bold text-foreground tracking-tight">Billing Cycles</h1>
        <p className="text-muted-foreground text-sm mt-0.5">Track your billing cycles and payments</p>
      </div>
      ...
      {cards.length > 0 && (
        <div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {cards.map((card) => {
              const { cycleStart, cycleEnd, dueDate: cycleDue } = getBillingCycleDates(card.billingDate, card.dueDate)
              ...
              return (
                <div key={card.id} className={cn(
                  ...
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {urgent && <AlertCircle className="w-4 h-4 text-rose-500" />}
                  </div>
                  <div className="space-y-1 text-xs">
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Cycle</span>
                      <span className="font-medium">{formatDate(cycleStart)} – {formatDate(cycleEnd)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Due Date</span>
                      <span className={cn('font-semibold', urgent ? 'text-rose-600' : 'text-foreground')}>
      ...
      {cycles.length > 0 ? (
        <div>
          ...
          <div className="space-y-3">
            {cycles.map((cycle) => {
              const outstanding = Number(cycle.totalSpend) - Number(cycle.paidAmount)
              const statusInfo = STATUS_MAP[cycle.paymentStatus as keyof typeof STATUS_MAP]
              const daysUntilDue = getDaysUntil(cycle.dueDate)
              const isUpcoming = daysUntilDue >= 0 && cycle.paymentStatus !== 'PAID'

              return (
                <div key={cycle.id} className="bg-white border border-border rounded-2xl overflow-hidden">
                  <div className="flex items-center justify-between p-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-semibold text-foreground">{cycle.card.nickname}</p>
                    ...
                    <div className="text-right">
                      <p className="text-lg font-bold text-foreground">{formatCurrency(cycle.totalSpend)}</p>
                      {outstanding > 0 && (
                        <p className="text-xs text-rose-600 font-medium">{formatCurrency(outstanding)} pending</p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="px-4 pb-4 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <span className={cn('text-xs font-semibold px-2 py-0.5 rounded-full', statusInfo.cls)}>
                        {statusInfo.label}
                      </span>
                      <span className={cn(
                        'text-xs',
                        isUpcoming && daysUntilDue <= 3 ? 'text-rose-600 font-semibold' : 'text-muted-foreground'
                      )}>
                        Due {formatDate(cycle.dueDate)}
                        {isUpcoming && ` (${daysUntilDue}d)`}
                      </span>
                      {cycle.paidAmount && Number(cycle.paidAmount) > 0 && (
                        <span className="text-xs text-emerald-600">Paid: {formatCurrency(cycle.paidAmount)}</span>
                      )}
                    </div>
                    {cycle.paymentStatus !== 'PAID' && outstanding > 0 && (
                      <RecordPaymentBtn cycleId={cycle.id} cardName={cycle.card.nickname} outstanding={outstanding} />
                    )}
                  </div>
                  ...
                  {cycle.transactions.length > 0 && (
                    <div className="border-t border-border px-4 py-3">
                      <p className="text-xs text-muted-foreground mb-2">{cycle.transactions.length} transaction{cycle.transactions.length !== 1 ? 's' : ''} this cycle</p>
                      <div className="space-y-1">
                        ...
                        {cycle.transactions.length > 3 && (
                          <p className="text-xs text-muted-foreground">+ {cycle.transactions.length - 3} more</p>
                        )}
                      </div>
                    </div>
                  )}
        ...
        <div className="text-center py-20">
          ...
          <p className="text-sm text-muted-foreground">Billing cycles are created automatically when you add transactions</p>
        </div>
      )}
    </div>
  )
