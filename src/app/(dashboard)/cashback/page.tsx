...
export default async function CashbackPage() {
      ...
      include: {
        card: {
          select: {
            ...
            nickname: true,
            bankName: true,
            cashbackCapType: true,
            cashbackCapAmount: true,
  ...
  const totalCredited = credited.reduce(
    (sum, r) => sum + Number(r.creditedAmount || 0),
    0,
  );
        ...
        {cashbackRecords.length === 0 ? (
          <div className="bg-white border border-border rounded-2xl p-10 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              Add transactions with cashback tracking to see them here
            </p>
          </div>
        ) : (
                  ...
                  return (
                    ...
                    >
                      <td className="px-5 py-3.5">
                        ...
                        <p className="text-xs text-muted-foreground">
                          Spent:{" "}
                          {formatCurrency(Number(record.transaction.amount))}
                        </p>
                      </td>
                      ...
                      <td className="px-5 py-3.5 text-right">
                        <p className="text-sm font-bold text-foreground">
                          {formatCurrency(
                            Number(
                              record.creditedAmount || record.expectedAmount,
                            ),
                          )}
                        </p>
                        {record.creditedAmount &&
                          Number(record.creditedAmount) !==
