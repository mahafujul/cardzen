...
export default async function DashboardPage() {
  ...
  const [cards, recentTx, pendingBorrowed, pendingCashback, upcomingCycles] = await Promise.all([
    ...
    prisma.cashbackRecord.findMany({
      ...
      include: { card: { select: { nickname: true } }, transaction: { select: { title: true } } },
      orderBy: { createdAt: 'desc' },
      take: 5,
    }),
    prisma.billingCycle.findMany({
      where: { card: { userId }, paymentStatus: { not: 'PAID' }, dueDate: { gte: now } },
      include: { card: true },
      orderBy: { dueDate: 'asc' },
      take: 5,
    }),
  ...
  const totalPendingBorrowed = pendingBorrowed.reduce(
    (sum, b) => sum + Number(b.amountOwed) - Number(b.amountReceived), 0
  )
  const totalCashbackPending = pendingCashback.reduce((sum, c) => sum + Number(c.expectedAmount), 0)
  const totalOutstanding = upcomingCycles.reduce(
    (sum, c) => sum + Number(c.totalSpend) - Number(c.paidAmount), 0
  )

  const CATEGORY_LABELS: Record<string, string> = {
    FOOD: '🍽️ Food', SHOPPING: '🛍️ Shopping', TRAVEL: '✈️ Travel',
    UTILITY: '⚡ Utility', BILLS: '📋 Bills', SUBSCRIPTION: '🔄 Subscriptions',
    MEDICAL: '🏥 Medical', FUEL: '⛽ Fuel', OTHERS: '📦 Others',
  }
  ...
  return (
    <div className="space-y-6 pb-8">
      ...
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-0.5">Your credit card overview</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          icon={spendDelta >= 0 ? <TrendingUp className="w-4 h-4" /> : <TrendingDown className="w-4 h-4" />}
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {upcomingCycles.length > 0 && (
            <div className="stat-card">
              <div className="flex items-center justify-between mb-4">
                ...
                <Link href="/billing" className="text-xs text-blue-600 hover:text-blue-500 flex items-center gap-1">
                ...
                {upcomingCycles.map((cycle) => {
                  ...
                  return (
                    <div key={cycle.id} className={cn(
                      ...
                      urgent ? 'bg-rose-50 border-rose-100' : 'bg-secondary/30 border-border'
                    )}>
                      <div className="flex items-center gap-3">
                        {urgent && <AlertCircle className="w-4 h-4 text-rose-500 flex-shrink-0" />}
                        {!urgent && <Clock className="w-4 h-4 text-muted-foreground flex-shrink-0" />}
                        <div>
                          <p className="text-sm font-semibold text-foreground">{cycle.card.nickname}</p>
                          <p className="text-xs text-muted-foreground">{cycle.card.bankName}</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className={cn('text-sm font-bold', urgent ? 'text-rose-600' : 'text-foreground')}>
                          {formatCurrency(amount)}
                        </p>
                        <p className={cn('text-xs', urgent ? 'text-rose-500' : 'text-muted-foreground')}>
          ...
          <div className="stat-card">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/transactions" className="text-xs text-blue-600 hover:text-blue-500 flex items-center gap-1">
                View all <ArrowRight className="w-3 h-3" />
              </Link>
            </div>
            {recentTx.length === 0 ? (
                ...
                {recentTx.map((tx) => (
                  <div key={tx.id} className="flex items-center justify-between py-2.5 border-b border-border last:border-0">
                    ...
                    <div className="text-right">
                      ...
                      {tx.borrowedExpense && (
                        <span className="text-xs text-violet-600 font-medium">Borrowed</span>
                      )}
                    </div>
                  </div>
                ))}
        ...
        <div className="space-y-5">
          ...
          <div className="stat-card">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/cards" className="text-xs text-blue-600 hover:text-blue-500 flex items-center gap-1">
            ...
            {cards.length === 0 ? (
              <div className="text-center py-6">
                ...
                <Link href="/cards/new" className="text-sm font-medium text-blue-600 hover:text-blue-500">
                  Add your first card →
                </Link>
              </div>
            ) : (
                ...
                {cards.slice(0, 4).map((card, i) => (
                  <div key={card.id} className={cn(
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-xs font-medium opacity-80">{card.bankName}</p>
                        <p className="text-sm font-bold">{card.nickname}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-xs opacity-80">•••• {card.lastFourDigits}</p>
                  ...
                  return (
                    <div key={cat.category}>
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-xs font-semibold text-foreground">{formatCurrency(amount)}</span>
                      </div>
          ...
          {pendingCashback.length > 0 && (
            <div className="stat-card">
              <div className="flex items-center justify-between mb-3">
                ...
                <Link href="/cashback" className="text-xs text-blue-600 hover:text-blue-500">
                  View all
                </Link>
              </div>
              <div className="space-y-2">
                {pendingCashback.slice(0, 3).map((cb) => (
                  <div key={cb.id} className="flex items-center justify-between py-2 border-b border-border last:border-0">
                    <div>
                      <p className="text-xs font-medium text-foreground">{cb.card.nickname}</p>
                      <p className="text-xs text-muted-foreground truncate max-w-[120px]">{cb.transaction.title}</p>
                    </div>
                    <span className="text-sm font-bold text-amber-600">{formatCurrency(cb.expectedAmount)}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {pendingBorrowed.length > 0 && (
            <div className="stat-card">
              <div className="flex items-center justify-between mb-3">
                ...
                <Link href="/borrowed" className="text-xs text-blue-600 hover:text-blue-500">
                  View all
                </Link>
              </div>
              <div className="space-y-2">
                {pendingBorrowed.slice(0, 3).map((b) => {
                  ...
                  return (
                    <div key={b.id} className="flex items-center justify-between py-2 border-b border-border last:border-0">
                      <div>
                        <p className="text-xs font-medium text-foreground">{b.personName}</p>
                        <span className={cn('text-xs', overdue ? 'text-rose-500' : 'text-muted-foreground')}>
                          {overdue ? 'Overdue' : b.paymentStatus === 'PARTIALLY_PAID' ? 'Partial' : 'Pending'}
                        </span>
                      </div>
                      <span className="text-sm font-bold text-violet-600">{formatCurrency(remaining)}</span>
                    </div>
                  )
                })}
              </div>
