...
import { Plus, Filter, ArrowLeftRight } from 'lucide-react'
    ...
    <div className="space-y-6 pb-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-0.5">{total} total transactions</p>
        </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border bg-secondary/30">
                  ...
                  <th className="text-left text-xs font-semibold text-muted-foreground px-3 py-3 hidden lg:table-cell">Date</th>
                  ...
                  return (
                    <tr key={tx.id} className="hover:bg-secondary/20 transition-colors">
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-1.5 mt-0.5">
                              ...
                              {tx.cashbackRecord && (
                                <span className={cn(
                                  'text-xs font-medium',
                                  tx.cashbackRecord.status === 'CREDITED' ? 'text-emerald-600' :
                                    tx.cashbackRecord.status === 'PENDING' ? 'text-amber-600' : 'text-muted-foreground'
                                )}>
                                  +{formatCurrency(tx.cashbackRecord.expectedAmount)} cashback
                                </span>
                              )}
                            </div>
                          </div>
                      ...
                      <td className="px-3 py-3.5 hidden sm:table-cell">
                        <p className="text-xs font-medium text-foreground">{tx.card.nickname}</p>
                        <p className="text-xs text-muted-foreground">{tx.card.bankName}</p>
                      </td>
                      ...
                      <td className="px-5 py-3.5 text-right">
                        ...
                        {tx.platformType === 'ONLINE' && tx.platformSource && (
                          <p className="text-xs text-muted-foreground">{tx.platformSource}</p>
                        )}
                      </td>
                    </tr>
                  )
