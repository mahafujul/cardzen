...
export async function GET(req: NextRequest) {
  ...
  const cashbackCapAlerts = await Promise.all(
    cards
      ...
      .map(async (card) => {
        ...
        if (
          card.cashbackCapType === "QUARTERLY_CAP" &&
          card.quarterStartMonth
        ) {
        ...
        const percentUsed = Math.min(
          100,
          Math.round((usedAmount / capAmount) * 100),
        );
  ...
  const annualFeeAlerts = await Promise.all(
    cards
      .filter(
        (c) =>
          c.feeType !== "LIFETIME_FREE" &&
          c.annualFeeAmount &&
          c.annualSpendWaiver,
      )
      .map(async (card) => {
        const yearStart = getAnnualSpendStart(
          card.feeRenewalMonth || now.getMonth() + 1,
        );
