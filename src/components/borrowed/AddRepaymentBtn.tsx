...
interface AddRepaymentBtnProps {
  borrowedId: string
  personName: string
  remaining: number
}

export default function AddRepaymentBtn({ borrowedId, personName, remaining }: AddRepaymentBtnProps) {
  ...
  return (
    <>
      <button
        ...
        className="flex items-center gap-1.5 text-xs font-semibold text-blue-600 hover:text-blue-500 px-3 py-1.5 rounded-lg hover:bg-blue-50 transition-colors"
      >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              From <strong>{personName}</strong> · {new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 }).format(remaining)} remaining
            </p>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium mb-1.5">Date Received *</label>
                <input
                  type="datetime-local"
                  value={receivedDate}
